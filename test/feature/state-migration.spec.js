...
describe('State migration', () => {
	...
	test('State is cleaned', () => {
		...
		const DEFAULT_STATE = {
			...
			user: {
				...
				settings: {
					...
					settingThatShouldBeRemoved: {
						// Notice there is no name or description, as the only thing saved was the value
						value: true
					}
				}
			}
		...
		// Act
		const CLEAN_STATE = cleanState(DEFAULT_STATE)
		...
		expect(CLEAN_STATE.version).toBe(1)
		expect(CLEAN_STATE).toEqual(EXPECTED_STATE)
	})
})
