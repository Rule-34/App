...
describe('Sitemap', async () => {
  ...
  it('does not duplicate locale prefixes in generated URLs', async () => {
    ...
    for (const localeCode of prefixedLocaleCodes) {
      expect(locs.some((loc) => new URL(loc).pathname.startsWith(`/${localeCode}/`))).toBe(true)
      expect(locs.some((loc) => new URL(loc).pathname.includes(`/${localeCode}/${localeCode}/`))).toBe(false)
    }
  })
})
