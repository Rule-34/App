...
describe('chunk error recovery', () => {
  ...
  it('retries a chunk failure once per URL in the current tab session', () => {
    const storage = new MemoryStorage()
    const key = buildChunkRecoveryStorageKey('https://r34.app/posts/rule34.xxx')

    expect(shouldAttemptChunkRecovery({ storage, key, online: true })).toBe(true)

    markChunkRecoveryAttempted({ storage, key, now: 12345 })

    expect(shouldAttemptChunkRecovery({ storage, key, online: true })).toBe(false)
  })
