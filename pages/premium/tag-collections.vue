...
const currentItem: Ref<ITagCollection> = ref(
  new TagCollection({
    name: '',
    tags: []
  })
...
function createItem() {
  ...
  // TODO: Design a better way to encode and decode tags
  tagCollections.value.push(new TagCollection({
...
<template>
  <main class='container flex flex-col mx-auto max-w-3xl flex-1 px-4 py-4 sm:px-6 lg:px-8'>
    <PageHeader>
    ...
    <section class='mx-2 mt-4 flex-auto'>
      ...
      >
        ...
        >
          ...
          <!-- Tag Length -->
          <div class='h-5 w-8 flex-shrink-0 overflow-x-hidden text-sm rounded-full tabular-nums px-2 bg-base-0/20 flex items-center justify-center'>
            {{ tagCollection.tags.length }}

            <span class='sr-only'>
              Tags in tag collection
            </span>
          </div>
  ...
  >
    <template #title>
      {{ dialogMode === 'create' ? 'Create' : 'Edit' }} tag collection
    </template>
