...
export default {
  ...
  computed: {
    ...
    boorusThatHaveSavedPosts() {
      const BOORU_DOMAIN_LIST = this.getCustomSavedPosts.map(
    ...
    savedPostsFromSelectedBooru() {
      if (this.selectedBooru === '<All Boorus>') {
        return JSON.parse(JSON.stringify(this.getCustomSavedPosts))
      }

      return this.getCustomSavedPosts.filter(
  ...
  methods: {
    ...
    onSearch(query) {
      const UNIQUE_TAGS_FROM_SAVED_POSTS = [
        ...new Set(
          this.savedPostsFromSelectedBooru
            .map((POST) => {
              let postTags = []
