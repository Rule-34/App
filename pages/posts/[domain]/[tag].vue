<script lang="ts" setup>
  ...
  import { fallbackBooruDomain, generatePostsRoute } from '~/assets/js/RouterHelper'
    ...
    () =>
      $fetch<IPostPage>(`/booru/${selectedBooru.value.type.type}/posts`, {
        ...
        params: {
          ...
          tags: tagParam.value,
          httpScheme: (selectedBooru.value.config?.options as any)?.HTTPScheme ?? undefined
        },
        retry: false
      }),
    {
  ...
  function getSingleRouteParam(param: string | string[] | undefined) {
  ...
  <main class="container mx-auto max-w-3xl flex-1 px-4 py-4 sm:px-6 lg:px-8">
    ...
    <div class="my-4 flex flex-wrap gap-2">
      <NuxtLink
        ...
        class="focus-visible:focus-outline-util hover:hover-bg-util hover:hover-text-util ring-base-0/20 rounded-md px-2 py-1 text-sm ring-1"
      >
      ...
      >
        ...
        >
          <NuxtLink
            ...
            :to="localePath(fullPostsRoute)"
            class="focus-visible:focus-outline-util hover:hover-bg-util hover:hover-text-util block h-full rounded-md"
          >
            <figure class="border-base-0/20 h-full overflow-hidden rounded-md border">
              <NuxtPicture
                ...
                :src="getPostPreviewFile(post).url"
              ...
              <div
                v-else
                class="bg-base-900 flex aspect-2/3 items-center justify-center px-2 text-center text-sm"
              >
                {{ t('posts.noResults') }}
              </div>

              <figcaption class="px-2 py-1.5 text-xs">
                <span class="truncate-clip block">{{ getPostPreviewAlt(post) }}</span>
                <span class="text-base-content/70 block">#{{ post.id }}</span>
              </figcaption>
            </figure>
          </NuxtLink>
        </li>
