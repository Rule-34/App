		...
		<main class="flex min-h-full flex-auto flex-col space-y-4 pb-4">
			<ul class="flex-auto space-y-4">
				...
				<template v-else-if="$fetchState.pending">
					<li class="my-3 text-center text-gray-300">Loading…</li>
				</template>
			</ul>
...
export default {
	...
	head() {
		...
		const ABSOLUTE_URL = new URL(RESOLVED_CANONICAL_ROUTE.href, window.location.origin).href
		...
		HEAD.meta.push({
			hid: 'og:image',
			property: 'og:image',
			content: `https://url-shot.api-point.cf/?width=1200&height=630&url=${encodeURI(ABSOLUTE_URL)}`
		})
	...
	methods: {
		...mapActions('booru', ['activeBooruManager', 'pidManager', 'tagsManager', 'fetchTags']),
