...
const selectedProvider = computed(() => {
  ...
  return pushAdProviders.find((provider) => provider.id === selectedRandomScript.value) ?? getPushAdProviderByKey('evadav')
})
...
watch(
  ...
  () => {
    ...
    selectedRandomScript.value = import.meta.client && providerMode.value === 'random' ? selectRandomPushAdProviderScript() : ''
    clearReport()
  },
  { immediate: true }
)
...
function patchNotificationPermission() {
  if (!('Notification' in window)) {
    addEvent('notification-permission-request', { permission: 'unsupported', message: 'Notification API unsupported' })
    return
  }
...
function installMutationObserver() {
  mutationObserver = new MutationObserver((records) => {
    ...
    const adLikeNodes = added.filter((node) => {
      ...
      return element.matches('iframe, ins, [id*="ad" i], [class*="ad" i]') || Boolean(element.querySelector('iframe, ins, [id*="ad" i], [class*="ad" i]'))
    })
