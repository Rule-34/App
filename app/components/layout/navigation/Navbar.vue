<script lang="ts" setup>
  import { Bars3Icon, XMarkIcon } from '@heroicons/vue/24/outline'
  import { useEventListener } from '@vueuse/core'

  const route = useRoute()
  const localePath = useLocalePath()
  const getRouteBaseName = useRouteBaseName()

  const { value: isMenuActive, toggle: toggleMenu } = useMenu()

  const { seasonalEmoji } = useSeasonalIcon()

  const isPageWithLogoDisabled = computed(() => getRouteBaseName(route) === 'index')

  const isPostsPage = computed(() => {
    const baseName = getRouteBaseName(route)

    return baseName === 'posts-domain' || baseName === 'premium-saved-posts-domain'
  })

  const isOnTop = ref(true)

  function updateIsOnTop() {
    isOnTop.value = window.scrollY <= 0
  }

  onMounted(() => {
    updateIsOnTop()
    useEventListener(window, 'scroll', updateIsOnTop, { passive: true })
  })
</script>

<template>
  <!-- Same margin as Nav height -->
  <nav class="mb-14">
    <div
      :class="{
        'fixed!': isPostsPage,
        'bg-base-1000/60 shadow-lg backdrop-blur-lg backdrop-saturate-200 md:border-b-2': isPostsPage && !isOnTop
      }"
      class="absolute inset-x-0 top-0 z-10 border-base-0/20 transition duration-200"
    >
      <!-- Navbar -->
      <div
        :class="[
          // Container
          'container mx-auto max-w-3xl sm:px-6 lg:px-8'
        ]"
        class="relative flex h-14 items-center justify-between"
      >
        <!-- -->

        <!-- Right side: Menu button -->
        <div class="absolute inset-y-0 left-0 flex items-center pl-2">
          <button
            :aria-label="isMenuActive ? $t('common.closeMenu') : $t('common.openMenu')"
            class="inline-flex items-center justify-center rounded-md p-2 text-base-content-highlight hover:hover-bg-util hover:hover-text-util focus-visible:focus-outline-util focus-visible:ring-inset"
            type="button"
            @click="toggleMenu()"
          >
            <Bars3Icon
              v-if="!isMenuActive"
              class="block h-6 w-6"
            />

            <XMarkIcon
              v-else
              class="block h-6 w-6"
            />
          </button>
        </div>

        <!-- Center: Logo -->
        <div class="flex flex-1 items-center justify-center">
          <NuxtLink
            v-if="!isPageWithLogoDisabled"
            :to="localePath('/')"
            class="shrink-0 focus-visible:focus-outline-util"
          >
            <img
              v-if="!seasonalEmoji"
              :alt="$t('common.icon')"
              class="flip-vertical-fwd h-6 w-6 text-base-content-highlight"
              height="24"
              src="/icon.svg"
              width="24"
            />

            <!-- Seasonal Icon -->
            <span
              v-else
              class="flip-vertical-fwd flex h-6 w-6 items-center justify-center text-2xl"
              style="mask-image: url('/icon.svg'); mask-size: contain; mask-repeat: no-repeat; mask-position: center"
            >
              {{ seasonalEmoji }}
            </span>
          </NuxtLink>
        </div>

        <!-- Right side: Actions -->
        <div
          id="navbar-actions"
          class="absolute inset-y-0 right-0 flex flex-row-reverse items-center pr-2"
        />
      </div>
    </div>
  </nav>
</template>
