...
/**
 * Global registry for coordinating playback across multiple video elements.
 * When any video begins playing, all other registered video elements are paused.
 */
...
export interface VideoPlaybackController {
  ...
  getPlayer?: () => { pause: () => void } | undefined | null
}

const activeVideoControllers = new Set<VideoPlaybackController>()
let globalPlayCaptureRegistered = false

interface GlobalDocumentLike {
  ...
  querySelectorAll: (selector: string) => Iterable<VideoElementLike>
}

function getDocument(): GlobalDocumentLike | undefined {
  if (typeof globalThis !== 'undefined' && 'document' in globalThis) {
    return (globalThis as unknown as { document: GlobalDocumentLike }).document
  }
  return undefined
}

function isVideoElement(el: unknown): el is VideoElementLike {
  if (!el || typeof el !== 'object') {
    return false
  }
  const candidate = el as { tagName?: string; pause?: unknown }
  return candidate.tagName === 'VIDEO' || typeof candidate.pause === 'function'
}

function onGlobalPlayCapture(event: unknown): void {
  const target = (event as { target?: unknown } | null | undefined)?.target
  if (!isVideoElement(target)) {
    return
  }

  // 1. Pause registered controllers with associated player wrappers
  for (const controller of activeVideoControllers) {
    if (controller.video !== target && !controller.video.paused) {
      pauseVideoPlayback(controller.video, controller.getPlayer?.())
    }
  }

  // 2. Also ensure any other playing video element in the DOM is paused
  ...
  if (doc) {
    try {
      ...
      // Ignore querySelectorAll failure in mock/unsupported environments
    }
  }
}

/**
 * Registers a video element in the global playback coordinator and attaches
 * a document-level capturing 'play' listener if not already initialized.
 *
 * @param video - The video element to register.
 * @param getPlayer - Optional callback returning the associated custom player instance.
 * @returns A cleanup function to unregister the video element.
 */
export function registerVideoPlayback(
  video: VideoElementLike,
  getPlayer?: () => { pause: () => void } | undefined | null
): () => void {
  const controller: VideoPlaybackController = { video, getPlayer }
  activeVideoControllers.add(controller)

  const doc = getDocument()
  if (!globalPlayCaptureRegistered && doc) {
    doc.addEventListener('play', onGlobalPlayCapture, true)
    globalPlayCaptureRegistered = true
  }

  return () => {
    activeVideoControllers.delete(controller)
  }
}

/**
 * Resets the global video playback coordinator and detaches event listeners.
 * Primarily used for testing isolation.
 */
export function resetVideoCoordinator(): void {
  activeVideoControllers.clear()
  if (globalPlayCaptureRegistered) {
    const doc = getDocument()
    if (doc) {
      try {
        doc.removeEventListener('play', onGlobalPlayCapture, true)
      } catch {
        // Ignore
      }
    }
    globalPlayCaptureRegistered = false
  }
}

/**
 * Pauses a video element and its associated fluid player instance if present.
 *
 * @param video - The video element to pause.
 * @param player - Optional FluidPlayer instance to pause.
 */
export function pauseVideoPlayback(video?: VideoElementLike | null, player?: { pause: () => void } | null): void {
  try {
    player?.pause()
  } catch {
    // Player pause could throw if in teardown
  }
